resource "azurerm_virtual_machine_scale_set" "user19vmss2" {
name = "user19vmss2"
...
os_profile {
    ...
    admin_username = "user19"  ## VM 에 접속할 계정
...
ssh_keys {
    path = "/home/user19/.ssh/authorized_keys"   ## pwd 실행후 경로설정 ex) /home/user01 등 
...
network_profile {
        ...
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user19-bpepool2.id]
        load_balancer_inbound_nat_rules_ids = [azurerm_lb_nat_pool.lbnatpool2.id]
    }
        network_security_group_id = azurerm_network_security_group.user19nsg.id
}
tags = {
    environment = "staging"
    }
